...
def create_song():
    ...
    song = db.songs.find_one({"id": input_song["id"]})
    if song:
        return {
            "Message": f"song with id {input_song['id']} already present"
        }, 302
    insert_id = db.songs.insert_one(input_song)
